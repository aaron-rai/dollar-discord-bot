...
DEFAULT_JOIN_CHANNEL_NAME = "JOIN HERE💎"

created_channels = set()


async def create_personal_channel(member: discord.Member, join_channel: discord.VoiceChannel) -> None:
	"""
	...
	async with lock_channel(join_channel):
		...
		if not member.voice or member.voice.channel != new_channel:
			...
			logger.warning(f"User did not join voice channel, deleted orphaned channel {new_channel.name} in {guild.name}")
			return


async def handle_channel_leave(channel: discord.VoiceChannel) -> None:
	"""
...
def get_join_channel(guild: discord.Guild, bot: discord.Client) -> discord.VoiceChannel:
	"""
	...
	if not guild_saved_channel:
		guild_saved_channel = DEFAULT_JOIN_CHANNEL_NAME
